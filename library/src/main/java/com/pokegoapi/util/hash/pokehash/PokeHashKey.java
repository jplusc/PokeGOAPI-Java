...
public class PokeHashKey {
	...
	@Getter
	private int keyExpiration;
	@Getter
	private int ratePeriodEnd;
	...
	synchronized void setProperties(HttpURLConnection connection) {
        ...
        // my additions. change long to int
		this.ratePeriodEnd = this.getHeaderInteger(connection, "X-RatePeriodEnd", this.ratePeriodEnd);
		...
		this.keyExpiration = this.getHeaderInteger(connection, "X-AuthTokenExpiration", this.keyExpiration);
		this.tested = true;
	}
