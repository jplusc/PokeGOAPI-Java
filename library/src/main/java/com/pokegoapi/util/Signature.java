...
public class Signature {
    ...
    // my additions for hash control
    public static boolean fallbackApi = false;
    public static Boolean validApiKey = null;
    // end additions
	...
	public static void setSignature(PokemonGo api, RequestEnvelope.Builder builder) throws RequestFailedException {
		...
		if (builder.hasAuthTicket()) {
            ...
            try {
                if (validApiKey == null) {
                    hash = testKey(currentTimeMillis, latitude, longitude, accuracy, authTicket, sessionHash, requestData, provider);
                } else if (!validApiKey) {
					if (PokeHashProvider.isKeyExpired()) throw new HashException("Your key is expired. You'll have to buy a new one to keep scanning :(");
                    else throw new HashException("Unauthorized key");
                } else {
                    hash = provider.provide(currentTimeMillis, latitude, longitude, accuracy, authTicket, sessionHash, requestData);
                }
            } catch (HashException hashException) {
                if (fallbackApi) {
                    hash = new LegacyHashProvider().provide(currentTimeMillis, latitude, longitude, accuracy, authTicket, sessionHash, requestData);
                } else {
					if (PokeHashProvider.isKeyExpired()) throw new HashException("Your key is expired. You'll have to buy a new one to keep scanning :(");
					else throw hashException;
                }
            }
            // end additions
    ...
    // my additions for hash key control
    private static synchronized Hash testKey(long currentTime, double latitude, double longitude, double accuracy, byte[] authTicket, byte[] sessionHash, byte[][] requestData, HashProvider provider) throws HashException {
        Hash hash = null;
        if (validApiKey != null && !validApiKey) throw new HashException("Unauthorized key");
        try {
            hash = provider.provide(currentTime, latitude, longitude, accuracy, authTicket, sessionHash, requestData);
            validApiKey = true;
            return hash;
