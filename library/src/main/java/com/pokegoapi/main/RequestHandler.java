...
public class RequestHandler implements Runnable {
    // my addition. make sure requests don't send too fast. otherwise get INVALID_PLATFORM_REQUEST
	...
	public Observable<ServerResponse> sendAsyncServerRequests(ServerRequestEnvelope envelope) {
		// This causes captchas to hang the entire thread. Just use the old way it worked fine
		...
		while (active) {
			...
			if (!workQueue.isEmpty()) {
				...
				try {
                    ...
                    // my additions to handle random errors (also changed ^ to Throwable)
                    if (!(e instanceof RequestFailedException)) {
                        response.setException(new RequestFailedException(e));
                    } else {
						if (e instanceof LoginFailedException) response.setException((LoginFailedException) e);
                        else if (e instanceof CaptchaActiveException) response.setException((CaptchaActiveException) e);
                        else if (e instanceof HashException) response.setException((HashException) e);
                    }
                }
                // end additions
				...
				try {
					...
					if (!(e instanceof RequestFailedException) && !(e instanceof InvalidProtocolBufferException)) {
						response.setException(new RequestFailedException(e));
					} else {
						if (e instanceof RequestFailedException) response.setException((RequestFailedException) e);
						else if (e instanceof InvalidProtocolBufferException) response.setException((InvalidProtocolBufferException) e);
					}
				}
