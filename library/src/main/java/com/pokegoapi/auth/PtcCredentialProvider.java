...
public class PtcCredentialProvider extends CredentialProvider {
	...
	private void login(String username, String password, int attempt)
		...
		try {
            ...
            // my additions, change this to use URLEncoder on username/pass
			HttpUrl url = HttpUrl.parse(LOGIN_URL).newBuilder()
					...
					.addEncodedQueryParameter("username", URLEncoder.encode(username, "UTF-8"))
					.addEncodedQueryParameter("password", URLEncoder.encode(password, "UTF-8"))
					.build();
	...
	@Override
	public void relogin() throws InvalidCredentialsException, LoginFailedException {
		login(username, password, 0);
    }
